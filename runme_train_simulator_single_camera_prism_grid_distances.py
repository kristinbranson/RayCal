...
def train_two_cams(model, train_loader, criterion, plot=False):    
    ...
    with torch.autograd.set_detect_anomaly(True):
        ...
        for i, (input, label_2D, label_3D, pairwise_distance_batch) in enumerate(train_loader):
            ...
            loss = 5 * recon_real_loss.sum()  + 0 * triangulation_loss + 1 * closest_distance_loss + 5e2 * pairwise_distance_loss + 1e3 * intersection_loss + 0 * distortion_loss
            loss.backward()
...
print(f'Distortion loss: {distortion_loss_test}')
