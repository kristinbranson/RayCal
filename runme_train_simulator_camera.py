...
T = torch.tensor(stereoParams['TranslationOfCamera2'][0,0]).to(torch.float64).T + 20.

principal_point_pixel_cam_0 = torch.tensor([640., 512.]) #torch.tensor([K1[0,2] - 1, K1[1,2] - 1], dtype=torch.float64)
principal_point_pixel_cam_1 = torch.tensor([640., 512.]) #torch.tensor([K2[0,2] - 1, K2[1,2] - 1], dtype=torch.float64)
...
class Arena(nn.Module):
    ...
    def forward(self, _, pixels_real_two_cams):
        ...
        recon_real_pixels = torch.cat((recon_distorted_real_pixels_cam_0,
                                    recon_distorted_real_pixels_cam_1), dim=0)        
...
def train_two_cams(model, train_loader, criterion, plot=False):    
    ...
    with torch.autograd.set_detect_anomaly(True):
        ...
        for input, label_2D, label_3D in train_loader:         
            ...
            optimizer.zero_grad()
            output, closest_distance, recon_distorted_pixels_1, recon_distorted_pixels_2, recon_real_loss = model(
                input.T,
                label_2D.T)
            if plot:                
            ...
            ground_truth_loss = recon_real_loss.mean()
            closest_distance_loss = closest_distance.mean()
            loss = recon_real_loss.mean() 
            loss.backward()
            optimizer.step()
            gt_loss += ground_truth_loss.item()
            dist_loss += closest_distance_loss.item()
            
    return gt_loss / num_batches, dist_loss / num_batches
...
def validate(model, val_dataloader, criterion):
    ...
    with torch.no_grad():
        for input, label_2D, label_3D in val_dataloader:
            num_batches += 1
            output, closest_distance, recon_distorted_pixels_1, recon_distorted_pixels_2, recon_real_loss = model(input.T, label_2D.T)
            ...
            num_batches += 1
    return val_gt_loss / num_batches, val_dist_loss / num_batches
...
num_epochs = 500
optimizer = optim.Adam(arena.parameters(), lr=2e-1
                       )
...
print(f'Test loss: {reprojection_error_test.mean()}')
