...
class Arena_fish_tank_pairwise_distances(nn.Module):
    ...
    def forward(self, pixels_two_cams):
        ...
        return recon_3D, closest_distance, pairwise_distance, intersection_penalty_1 + intersection_penalty_2 + intersection_penalty_3


    def visualize(self, pixels_two_cams, color_labels=None):
        num_samples = 10
        undistorted_pixels_cam_0 = torch.hstack(
            (pixels_two_cams[:2, :].clone(), pixels_two_cams[2:4, :].clone())
        )
        undistorted_pixels_cam_1 = torch.hstack(
