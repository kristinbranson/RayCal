...
import datetime
import time
from arenas.prism_arenas import Arena_reprojection_loss_single_camera_prism
...
recon_3D_test, closest_dist_test, recon_distorted_pixels_1, dist_penalty_1, int_penalty_1 = arena(pixels_virtual_two_cams_test, 
...
recon_real_loss = euclidean_distance(
                recon_distorted_pixels_1, 
                pixels_real_two_cams_test[:2,:]).mean() 

print(f'Triangulation loss: {triangulation_loss}')
print(f'Distortion penalty: {(dist_penalty_1).mean()}')
print(f'Real pixel loss: {recon_real_loss.mean()}')

reprojection_error_1 = euclidean_distance(
    recon_distorted_pixels_1, pixels_real_two_cams_test[:2,:]
)

print(f'Reprojection error for two cameras: {reprojection_error_1.mean()}')

plt.figure()
plt.scatter(
    recon_distorted_pixels_1[0,:].detach().numpy(),
    recon_distorted_pixels_1[1,:].detach().numpy(),
    color='g',
    marker='o',
    label='Estimate',
)
